...
var STACK_BASE = 15830800, STACK_MAX = 21073680, DYNAMIC_BASE = 21073680, DYNAMICTOP_PTR = 15830768;
...
var wasmBinaryFile = "https://akami-milwaukeehub.azureedge.net/ve-data/caching/Milwaukee_World-101.wasm";
...
var UE_JSMilwaukeeHTML5 = {
 JS_SaveLikedProduct: function(x) {
  if (typeof window.VE !== "undefined") {
   window.VE.SaveProductLiked(x);
  }
 },
 JS_RequestMeeting: function(x) {
  if (typeof window.VE !== "undefined") {
   window.VE.SaveMeetingRequested(x);
  }
 },
 JS_MilwaukeeHTML5Loaded: function() {
  if (typeof window.VE !== "undefined") {
   window.VE.setVELoaded();
  }
  console.log("VE LOADED");
 },
 JS_DownloadProductAsset: function(x) {
  if (typeof window.VE !== "undefined") {
   window.VE.DownloadAsset(x);
  }
 },
 JS_GetCultureCode: function() {
  if (typeof window.VE !== "undefined") {
   var CultureCode = window.VE.GetCultureCode();
   if (typeof CultureCode !== "undefined") {
    this.UE_GetCultureCodeCallBack(CultureCode);
   }
  } else {
   this.UE_GetCultureCodeCallBack("VE NOT CONNECTED WITH  REACT");
  }
 },
 JS_GetPauseStatus: function() {
  if (typeof window.VE !== "undefined") {
   var PauseStatus = window.VE.GetPauseStatus();
   if (typeof PauseStatus !== "undefined") {
    this.UE_GetPauseStatusCallBack(PauseStatus);
   }
  } else {
   this.UE_GetPauseStatusCallBack(false);
  }
 },
 JS_GetTpStatus: function() {
  if (typeof window.VE !== "undefined") {
   var location = window.VE.GetTpStatus();
   if (typeof location !== "undefined") {
    this.UE_GetTpStatusCallBack(location);
   }
  }
 },
 JS_ResetTpLocation: function() {
  if (typeof window.VE !== "undefined") {
   window.VE.ResetTpLocation();
  }
 },
 JS_TrackVisitedWorkshop: function(x) {
  if (typeof window.VE !== "undefined") {
   window.VE.SaveWorkshopVisited(x);
  }
 },
 JS_TrackProductInteractedInVe: function(x) {
  if (typeof window.VE !== "undefined") {
   window.VE.SaveProductInteractedInVe(x);
  }
 }
};

function _UE_Download_Product_Asset(x) {
 UE_JSMilwaukeeHTML5.JS_DownloadProductAsset(x);
}
...
function _UE_Get_Culture_Code() {
 UE_JSMilwaukeeHTML5.JS_GetCultureCode();
}

function _UE_Get_Pause_Status() {
 UE_JSMilwaukeeHTML5.JS_GetPauseStatus();
}

function _UE_Get_Tp_Status() {
 UE_JSMilwaukeeHTML5.JS_GetTpStatus();
}

function _UE_Like_Product(x) {
 UE_JSMilwaukeeHTML5.JS_SaveLikedProduct(x);
}
...
var UE_JSMedia_HTML = {
 UE_Media_ControlVolumen: function(id, Volumen) {
  if (typeof UE_JSMedia_HTML.videos !== "undefined" && typeof UE_JSMedia_HTML.videos[id] !== "undefined" && typeof UE_JSMedia_HTML.videos[id].video !=='undefined') {
    UE_JSMedia_HTML.videos[id].video.volume = Volumen;
  } else {
    console.log("No Videos found",id);
  }
 },
 UE_Media_PlayFromTags: function(PlayerId, TagsOptions) {
  if (typeof UE_JSMedia_HTML.videos === "undefined") {
   UE_JSMedia_HTML.videos = [];
  }
  function Video(video, tex) {
   this.video = video;
   this.tex = tex;
   this.interval = null;
  }
  if (typeof GL === "undefined") {
   throw new Error("Run in an UE4 HTML5 game environment.");
  } else {
   console.log(TagsOptions);
   const Tags = TagsOptions.split("/");
   const Workshop = Tags[0];
   const VideoType = Tags[1];
   const Location = Tags[2];
   console.log(Workshop, VideoType, Location);
   window.API.Workshop.getWorkshopVideos(window.DATA.eventId, {
    workStation: Workshop,
    cultureCode: window.DATA.cultureCode
   }).then(response => {
    if (response.success) {
     const videos = response.data.data.event_workstation.videos.filter(video => {
      if (video.type === VideoType && video.tags.includes(Workshop + "-" + Location)) {
       return video;
      }
      return null;
     });
     if (typeof videos !== "undefined" && typeof videos[0] !== "undefined") {
      const videoDownload = videos[0].downloads.filter(file => file.height === 720 || file.height == 506 );
      console.log(videoDownload)
      if (typeof videoDownload !== "undefined" && typeof videoDownload[0] !== "undefined") {
       const videoUrl = videoDownload[0].link;
       if (typeof videoUrl !== "undefined") {
        UE_JSMedia_HTML.GL = GL;
        UE_JSMedia_HTML.getTexture(PlayerId);
        UE_JSMedia_HTML.initShaders();
        console.log(VideoType)
        if(VideoType==='VT-GS' || VideoType==='VT-TS'){
          if(VideoType==='VT-TS'){
            const analytics = window.analytics
            analytics.track('Testimonial Interacted', {
              videoUrl: videoUrl
            })
          }
          UE_JSMedia_HTML.initVideo(PlayerId, videoUrl,false,false);
        }else{
          UE_JSMedia_HTML.initVideo(PlayerId, videoUrl,true,true);
        }
        UE_JSMedia_HTML.videos[PlayerId] = new Video(UE_JSMedia_HTML.video, UE_JSMedia_HTML.tex);
        UE_JSMedia_HTML.render(PlayerId);
       }
      } else {
        window.API.Workshop.getWorkshopVideos(window.DATA.eventId, {
          workStation: Workshop,
            ...
            const videos = response.data.data.event_workstation.videos.filter(video => {
             ...
             if (typeof videos !== "undefined" && typeof videos[0] !== "undefined") {
              ...
              if (typeof videoDownload !== "undefined" && typeof videoDownload[0] !== "undefined") {
               ...
               console.log("Error Getting the VideoLink");
              }
             }
           }
         })
      ...
      console.log("Error Getting the ProductGallery");
     }
    } else {
     console.log("Error Getting the Workshop videos");
    }
   });
  }
 },
 UE_Media_PlayProductVideoFromId: function(ProductId) {
  const PlayerId = 319;
  if (typeof UE_JSMedia_HTML.videos === "undefined") {
   UE_JSMedia_HTML.videos = [];
  }
  ...
  if (typeof GL === "undefined") {
   ...
   window.API.product.GetProductDetails(window.DATA.eventId, {
    cultureCode: window.DATA.cultureCode,
    productId: ProductId
   }).then(response => {
    if (response.success) {
     const productGallery = response.data.data.productGallery.filter(data => data.type === "VT-PM");
     if (typeof productGallery !== "undefined" && typeof productGallery[0] !== "undefined") {
      const videoDownload = productGallery[0].downloads.filter(file => file.height === 720);
      if (typeof videoDownload !== "undefined" && typeof videoDownload[0] !== "undefined") {
       const videoUrl = videoDownload[0].link;
       UE_JSMedia_HTML.GL = GL;
       UE_JSMedia_HTML.getTexture(PlayerId);
 ...
 UE_Media_PauseVideo: function(PlayerId) {
  if (typeof GL === "undefined") {
   throw new Error("Run in an UE4 HTML5 game environment.");
  } else {
   if (typeof UE_JSMedia_HTML.videos !== "undefined") {
    if (typeof UE_JSMedia_HTML.videos[PlayerId] !== "undefined") {
     if (typeof UE_JSMedia_HTML.videos[PlayerId].video !== "undefined") {
      try {
        UE_JSMedia_HTML.videos[PlayerId].video.pause();
      }catch(error){
        console.log(error)
        console.log('WEBGLRROR')
      }
     }
    }
   }
 ...
 UE_Media_ResumeVideo: function(PlayerId) {
  if (typeof GL === "undefined") {
   throw new Error("Run in an UE4 HTML5 game environment.");
  } else {
   if (typeof UE_JSMedia_HTML.videos !== "undefined") {
 ...
 getTexture: function(id) {
  UE_JSMedia_HTML.textures = [];
  var GL = UE_JSMedia_HTML.GL;
  for (var i = 0; i < GL.textures.length; i++) {
   if (GL.textures[i] !== null) {
    UE_JSMedia_HTML.textures.push(GL.textures[i]);
   }
  }
  for (var i = 0; i < UE_JSMedia_HTML.textures.length; i++) {
   if (UE_JSMedia_HTML.textures[i].name === id) {
    UE_JSMedia_HTML.tex = UE_JSMedia_HTML.textures[i];
    break;
   }
  }
  console.log(UE_JSMedia_HTML.tex);
 },
 initShaders: function() {
  var GL = UE_JSMedia_HTML.GL;
  var glctx = GLctx;
  UE_JSMedia_HTML.glctx = glctx;
  var vs = glctx.createShader(glctx.VERTEX_SHADER);
  glctx.shaderSource(vs, "attribute vec2 vx;varying vec2 tx;void main(){gl_Position=vec4(vx.x*2.0-1.0,1.0-vx.y*2.0,0,1);tx=vx;}");
  glctx.compileShader(vs);
  var ps = glctx.createShader(glctx.FRAGMENT_SHADER);
  glctx.shaderSource(ps, "precision mediump float;uniform sampler2D sm;varying vec2 tx;void main(){gl_FragColor=texture2D(sm,tx);}");
  glctx.compileShader(ps);
  var shader = glctx.createProgram();
  glctx.attachShader(shader, vs);
  glctx.attachShader(shader, ps);
  glctx.linkProgram(shader);
  glctx.useProgram(shader);
  var vx_ptr = glctx.getAttribLocation(shader, "vx");
  glctx.enableVertexAttribArray(vx_ptr);
  glctx.uniform1i(glctx.getUniformLocation(shader, "sm"), 0);
  var vx = glctx.createBuffer();
  glctx.bindBuffer(glctx.ARRAY_BUFFER, vx);
  glctx.bufferData(glctx.ARRAY_BUFFER, new Float32Array([ 0, 0, 1, 0, 1, 1, 0, 1 ]), glctx.STATIC_DRAW);
  var ix = glctx.createBuffer();
  glctx.bindBuffer(glctx.ELEMENT_ARRAY_BUFFER, ix);
  glctx.bufferData(glctx.ELEMENT_ARRAY_BUFFER, new Uint16Array([ 0, 1, 2, 0, 2, 3 ]), glctx.STATIC_DRAW);
  UE_JSMedia_HTML.vs = vs;
  UE_JSMedia_HTML.ps = ps;
  UE_JSMedia_HTML.shader = shader;
  UE_JSMedia_HTML.vx_ptr = vx_ptr;
  UE_JSMedia_HTML.vx = vx;
  UE_JSMedia_HTML.ix = ix;
 },
 initVideo: function(id, url,muted,loop) {
  if (typeof UE_JSMedia_HTML.videos[id] !== "undefined" && typeof UE_JSMedia_HTML.videos[id].video !== "undefined") {
   UE_JSMedia_HTML.videos[id].video.pause();
  }
  ...
  UE_JSMedia_HTML.video = video;
 },
 render: function(id) {
  try {
    UE_JSMedia_HTML.videos[id].video.play();
  }catch(error){
    console.log(error)
    console.log('WEBGLRROR')
  }
  /* UE_JSMedia_HTML.videos[id].interval = setInterval(() => {
   if (UE_JSMedia_HTML.videos[id].video.paused) {
    clearInterval(UE_JSMedia_HTML.videos[id].interval);
    return;
   } else if (UE_JSMedia_HTML.videos[id].video.readyState !== 4) {
    return;
   }
   if (typeof UE_JSMedia_HTML.videos[id].tex !== "undefined") {
    var tex = UE_JSMedia_HTML.videos[id].tex;

     UE_JSMedia_HTML.updateTexture(UE_JSMedia_HTML.glctx,tex, UE_JSMedia_HTML.videos[id].video);
   }
  },5 );
  */
 },
 updateTexture:function (glctx, texture, video) {
...
function _UE_Media_ControlVolumen(PlayerId, volumen) {
 UE_JSMedia_HTML.UE_Media_ControlVolumen(PlayerId, volumen);
}

function _UE_Media_PauseVideo(PlayerId) {
 UE_JSMedia_HTML.UE_Media_PauseVideo(PlayerId);
}

function _UE_Media_PlayFromTags(PlayerId, TagsOptionschar) {
 UE_JSMedia_HTML.UE_Media_PlayFromTags(PlayerId, UTF8ToString(TagsOptionschar));
}

function _UE_Media_PlayProductVideoFromId(ProductId) {
 UE_JSMedia_HTML.UE_Media_PlayProductVideoFromId(ProductId);
}

function _UE_Media_ResumeVideo(PlayerId) {
 UE_JSMedia_HTML.UE_Media_ResumeVideo(PlayerId);
}
...
function _UE_MilwaukeeHTML5_Loaded(x) {
 UE_JSMilwaukeeHTML5.JS_MilwaukeeHTML5Loaded();
}

function _UE_RegisterGetCultureCodeCallBack(listener) {
 UE_JSMilwaukeeHTML5.UE_GetCultureCodeCallBack = function(str) {
...
function _UE_RegisterGetPauseStatusCallBack(listener) {
 UE_JSMilwaukeeHTML5.UE_GetPauseStatusCallBack = function(status) {
  dynCall("vi", listener, [ status ]);
 };
}

function _UE_RegisterGetTpStatusCallBack(listener) {
 UE_JSMilwaukeeHTML5.UE_GetTpStatusCallBack = function(location) {
  var cname = _malloc(location.length + 1);
  stringToUTF8(location, cname, location.length + 1);
  dynCall("vi", listener, [ cname ]);
 };
}

function _UE_RequestMeeting(x) {
 UE_JSMilwaukeeHTML5.JS_RequestMeeting(x);
}

function _UE_Reset_Tp_Location() {
 UE_JSMilwaukeeHTML5.JS_ResetTpLocation();
}
...
function _UE_TrackProductInteractedInVe(x) {
 UE_JSMilwaukeeHTML5.JS_TrackProductInteractedInVe(x);
}

function _UE_TrackVisitedWorkshop(x) {
 UE_JSMilwaukeeHTML5.JS_TrackVisitedWorkshop(x);
}
...
var ___tm_current = 15830624;

var ___tm_timezone = (stringToUTF8("GMT", 15830672, 4), 15830672);
...
var asmLibraryArg = {
 ...
 "_UE_Download_Product_Asset": _UE_Download_Product_Asset,
 ...
 "_UE_Get_Culture_Code": _UE_Get_Culture_Code,
 "_UE_Get_Pause_Status": _UE_Get_Pause_Status,
 "_UE_Get_Tp_Status": _UE_Get_Tp_Status,
 "_UE_Like_Product": _UE_Like_Product,
 "_UE_LoadGame": _UE_LoadGame,
 "_UE_MakeHTTPDataRequest": _UE_MakeHTTPDataRequest,
 "_UE_Media_ControlVolumen": _UE_Media_ControlVolumen,
 "_UE_Media_PauseVideo": _UE_Media_PauseVideo,
 "_UE_Media_PlayFromTags": _UE_Media_PlayFromTags,
 "_UE_Media_PlayProductVideoFromId": _UE_Media_PlayProductVideoFromId,
 "_UE_Media_ResumeVideo": _UE_Media_ResumeVideo,
 "_UE_MessageBox": _UE_MessageBox,
 "_UE_MilwaukeeHTML5_Loaded": _UE_MilwaukeeHTML5_Loaded,
 "_UE_RegisterGetCultureCodeCallBack": _UE_RegisterGetCultureCodeCallBack,
 "_UE_RegisterGetPauseStatusCallBack": _UE_RegisterGetPauseStatusCallBack,
 "_UE_RegisterGetTpStatusCallBack": _UE_RegisterGetTpStatusCallBack,
 "_UE_RequestMeeting": _UE_RequestMeeting,
 "_UE_Reset_Tp_Location": _UE_Reset_Tp_Location,
 "_UE_SaveGame": _UE_SaveGame,
 "_UE_SendAndRecievePayLoad": _UE_SendAndRecievePayLoad,
 "_UE_TrackProductInteractedInVe": _UE_TrackProductInteractedInVe,
 "_UE_TrackVisitedWorkshop": _UE_TrackVisitedWorkshop,
...
function step(timestamp) {
  ...
  const elapsed = timestamp - start;
    ...
    if(typeof UE_JSMedia_HTML.videos !== 'undefined'){
     UE_JSMedia_HTML.videos.forEach((video,index)=>{
       if(typeof video.video !== 'undefined' && typeof video.tex !== 'undefined'){
         if(!video.video.paused && video.video.readyState===4){
          UE_JSMedia_HTML.updateTexture(UE_JSMedia_HTML.glctx,video.tex, video.video);
         }
       }
     })
    }
