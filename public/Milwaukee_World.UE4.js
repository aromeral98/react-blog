...
console.log("Emscripten version: D:/UE_4.20/UE_4.23/Engine/Extras/ThirdPartyNotUE/emsdk/emscripten/1.38.31");
console.log("Emscripten configuration: D:/UE_4.20/UE_4.23/Engine/Intermediate/Build/HTML5/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_Milwaukee_World', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202106271709, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../Milwaukee_World/Milwaukee_World.uproject','-stdout',];
...
function openIndexedDB(dbName, dbVersion) {
	return new Promise(function(resolve, reject) {
		...
		openRequest.onsuccess = function(evt) {
			window.VE.setLoadingFromCache(true);
			resolve(evt.target.result);
		};
...
function download(url, responseType) {
	return new Promise(function(resolve, reject) {
		...
		xhr.onprogress = function(p) {
			if(p.target.responseURL.includes('data.gz')){
				window.VE.setSizeLoaded(p.loaded)
			}
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// WASM
		var mainCompiledCode = download(Module.locateFile('https://akami-milwaukeehub.azureedge.net/ve-data/caching/Milwaukee_World-101.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://akami-milwaukeehub.azureedge.net/ve-data/caching/Milwaukee_World-101.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://akami-milwaukeehub.azureedge.net/ve-data/caching/Milwaukee_World-101.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://akami-milwaukeehub.azureedge.net/ve-data/caching/Utility-101.js')).then(addScriptToDom);
		var dataDownload =
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('Milwaukee_World.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('Milwaukee_World.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://akami-milwaukeehub.azureedge.net/ve-data/caching/Milwaukee_World-101.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://akami-milwaukeehub.azureedge.net/ve-data/caching/Milwaukee_World-101.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
